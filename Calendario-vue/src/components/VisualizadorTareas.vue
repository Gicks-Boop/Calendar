  ...
  <div class="visualizador-tareas">
    ...
    >
      ...
      <div class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-4 py-3 flex justify-between items-center">
        ...
        <button @click="cerrarPanel" class="text-white hover:text-gray-200 focus:outline-none transition-colors duration-200">
    ...
    >
      ...
      >
        ...
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 mb-2">Selecciona el mes a borrar</label>
        ...
        <p class="text-red-600 font-medium mb-6">¿Estás seguro de que deseas eliminar TODAS las tareas de {{ mesesDelAño[mesBorrar] }} {{ anioBorrar }}? Esta acción no se puede deshacer.</p>
...
export default {
  ...
  computed: {
    tareasPorCategoria() {
      const grupos = {};
      this.tareasPorCategoria.forEach(tarea => {
        const categoria = tarea.categoria || 'sin-categoria;'
      if(!grupos [categoria]){
        grupos[categoria] = {
          nombre: this.obtenerNombreCategoria(categoria),
          color: this.obtenerColorCategoria(categoria),
          tareas: []
        };
      }
      grupos[categoria].tareas.push(tarea);
      });
      return grupos;
    },
    contadorTareas() {
  ...
  methods: {
    ...
    eliminarTareasMes() {
      ...
      if (fechasAEliminar.length > 0) {
        ...
        this.mostrarToast(`Tareas del mes ${this.mesesDelAño[this.mesBorrar]} ${this.anioBorrar} eliminadas`, 'success');
      } else {
        this.mostrarToast(`No hay tareas para eliminar en ${this.mesesDelAño[this.mesBorrar]} ${this.anioBorrar}`, 'info');
      }
    ...
    obtenerColorCategoria(categoriaValor){
      const colorEncontrado = this.categorias.find(cat => cat.valor === categoriaValor);
      return colorEncontrado ? colorEncontrado.color : '#CCCCCC';
    },
